		...
		<form className="add-source-form tmdb-list-form" data-tmdb-list-stage={step} onSubmit={submit} noValidate>
			<div ref={scrollRef} className="add-source-scroll" inert={preview || undefined} aria-hidden={preview ? "true" : undefined}>
				{step === "select" ? <>
					...
					<div className="add-source-section-heading"><div><p className="panel-kicker">{standalone ? "Review" : "Review & Appearance"}</p><h3 ref={headingRef} id="tmdb-list-review-title" tabIndex={-1}>{count ? `${count} source${count === 1 ? "" : "s"} will be added` : "Nothing to add"}</h3></div></div>
