...
export function orderedSourceSortIds(sortOptionIds, sortOptionId = DEFAULT_DISCOVER_SORT_OPTION_ID) {
	const supplied = sortOptionIds === undefined ? [sortOptionId] : sortOptionIds;
	if (!Array.isArray(supplied) || supplied.length === 0
		|| new Set(supplied).size !== supplied.length
		|| [...supplied].some((id) => !DISCOVER_SORT_OPTIONS.some((option) => option.id === id))) return null;
	return Object.freeze(DISCOVER_SORT_OPTIONS.filter((option) => supplied.includes(option.id)).map((option) => option.id));
}
