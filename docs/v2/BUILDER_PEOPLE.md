...
Issue [#200](https://github.com/davecollections/tmdb-id-lookup/issues/200) has owner-approved implementation as of 2026-09-10; merge remains a separate owner gate. In Add Source, New Collection and New Folder, **Sources to create** selects any of Popular, Recent, Top rated and Most voted, initially Popular; empty selection blocks creation. Each eligible Acting/Directing Movie/Series combination gets one native scalar source per selected option, preserving Automatic, shared and per-person configuration. Multiple options append ` - <option>` to the existing base title. Preview now selects one exact Role/Media/Show candidate, including existing or omitted variants, reusing loaded combined credits. Most voted explicitly orders by vote count; distinct role/media totals include titles without posters. The scalar editor remains **Sort titles by**. The [native-variant contract](./BUILDER_NATIVE_VARIANTS.md) supersedes earlier single-sort, merged-role Preview and structural-only duplicate descriptions below. Guided New Folder plans exact Sources across the destination Collection: create a folder for an entity with no recognized matching folder, append missing variants to one matching folder, and add nothing for fully present entities. When several folders match and Sources are missing, Configure requires an inline **Add new sources to** choice. Existing Sources stay in place; names, artwork, raw data and settings remain preserved. Append-only plans use Add sources in Configure; Appearance affects new folders only. No-addition and unresolved plans cannot progress. See the [supplied client comparisons and recorded review evidence](../../manual-tests/native-source-variants/README.md), including website Most voted rewriting and direct-TV/Desktop preservation; displayed ranking acceptance remains separate.
...
New hierarchy selections cannot contain duplicate selected person IDs. Guided New Folder plans exact Sources across the destination Collection: create a folder for an entity with no recognized matching folder, append missing variants to one matching folder, and add nothing for fully present entities. When several folders match and Sources are missing, Configure requires an inline **Add new sources to** choice. Existing Sources stay in place; names, artwork, raw data and settings remain preserved. Exact matches elsewhere remain informational. New Collection creates all selected variants; ordinary Add retains its folder-local missing/add-all behavior. Per-person role/media pills disable only when all currently selected sort variants are already present, preserve intent, and re-enable as missing options are chosen.
