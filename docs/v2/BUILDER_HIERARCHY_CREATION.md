...
## Creation sorting and authored punctuation (#198)

For Streaming, Genres and Decades/Years, creation may hold an ordered set of semantic sorts; saved Sources and physical Source editors remain scalar. Expand validated candidates inside the family's existing grouping rules and apply one revalidated atomic plan. Do not create sort-specific Folders. People, Studios and Networks retain their existing single-sort behavior until separately scoped work; Franchises are excluded and TMDB Lists retain original order.

Creation controls permit an explicit empty set, show “Choose at least one option.” and prevent creation. Display “Choose one or more options. Movies and Series get separate sources.” once. Keep the established nested title Preview, with independent single-choice **Show** and unique Media selectors. Resolve exact candidates with every applicable dimension; preserve valid navigation choices, bound queries and caches, and make no automatic title requests when creation sorting changes. Samples remain explicitly distinct from exact Source results.

**Authored punctuation convention:** Builder-authored visible copy, accessibility copy and generated names must not use em dashes. Use an ASCII hyphen with spaces for separators, or natural wording. This rule applies to authored presentation only: preserve custom/imported titles, external metadata, URLs, identifiers, raw snapshots and serialized user content exactly. Do not apply blanket replacements to data.

Issue [#198](https://github.com/davecollections/tmdb-id-lookup/issues/198) has completed owner review and is approved for publication; merge approval remains pending. For its Preview interaction/request matrix, Dave explicitly requested controlled responses. That scoped matrix uses actual components/providers/request coordination with injected transport and controlled image responses; it is not live external-service evidence and does not change the general testing policy in docs/TESTING.md.

Approved visible wording: label the included multiple-choice creation group **Sources to create**, with the helper “Choose one or more options. Movies and Series get separate sources.” Associate its helper and “Choose at least one option.” validation with that group. Equivalent media/sort/Advanced introductions use “Choose your media, sources and any advanced settings, then review.” Summaries introduce selected variants with **Selected:**. Creation Preview labels its single-choice variant selector **Show** (accessible name: “Preview show”), retaining the active-results description. Physical Source editors keep **Sort titles by**. These are context-specific wording changes; generated names, internal sort IDs, option labels/order and deferred-family wording remain unchanged.
...
For People, Studios and Networks, the current [#200 native-variant contract](./BUILDER_NATIVE_VARIANTS.md) governs exact comparison and guided New Folder: collection-wide exact Source skipping, missing-Source appends to recognized folders, inline resolution of ambiguous append targets, and new folders only where needed. Append-only plans finish in Configure; Appearance applies only to new folders. The same existing atomic Collection extension operation handles mixed batches. Other families retain their focused placement contracts.
