...
Selected buttons use `aria-pressed`, a teal/cyan surface and border, and a structural inset treatment so selection is not communicated by hue alone. The workspace hierarchy does not use a leading selection rail. The source list preserves the exact authoritative source order and labels each row from the source's explicit `native-tmdb`, `addon`, or `opaque` category.
...
## Approved consistency direction

The following remains approved consistency direction:

- teal/cyan means selected, active, or current; green is meaningful success; amber/orange is warning or needs review; red is error or destructive; muted grey is disabled or unavailable;
- routine valid rows use counts where useful and normally omit **Ready to create** / **Ready to add** decoration;
- a full selected row uses **Remove** rather than an ambiguous `×`; tiny removable chips may retain `×` where its compact meaning remains clear;
- source-preview actions use **Preview titles**;
- edit footers prefer the primary action followed by Cancel, while final actions retain contextual labels such as **Save changes**, **Apply changes**, **Create collection**, **Create folder**, and **Add N sources**;
- established Nuvio user-facing terminology includes **Focus GIF**; raw runtime values such as `TV`, sort strings, and source types are humanized when shown to users;
- the dense three-column `Collections → Folders → Sources` desktop workspace remains; clicking a row primarily selects it and editing stays explicit; do not substantially increase workspace spacing.

Creation and edit dialogs already refuse backdrop dismissal. There is no dirty-state confirmation because project mutation occurs only on final Save/Create/Apply; interim state is dialog-local. View-only Preview backdrop behavior remains a separate unresolved consistency decision. Wizard stage meanings and the rule that Preview is not a stage are owned by [`BUILDER_HIERARCHY_CREATION.md`](./BUILDER_HIERARCHY_CREATION.md).

## Selectable-choice presentation contract

This section is the authoritative Builder rule for visually presenting selectable choices. [`BUILDER_HIERARCHY_CREATION.md`](./BUILDER_HIERARCHY_CREATION.md) applies it to guided family flows; `AGENTS.md` contains the concise implementation guard.

- Immediate launcher cards navigate or start an action. They are plain buttons and never gain retained radio/checkbox semantics, selected styling, or a selection marker.
- A full-card single choice uses a native radio for semantics and checked state, visually hides that radio, and puts the visible focus ring on the complete label/card. The selected card uses the established teal/cyan surface and border plus a structural inset. It does not show a radio dot or decorative tick.
- Poster/Landscape shape choices follow the same radio rule. Selection also strengthens the schematic shape preview, without adding a tick.
- Compact pills, segmented controls, tabs, and retained-choice buttons communicate state through their selected surface/border/inset only. Buttons expose `aria-pressed`; tabs expose `aria-selected`; hidden semantic inputs remain focusable where used. They do not change plus to tick or add a radio dot/tick.
- A multi-select entity card uses a native checkbox for semantics and checked state, visually hides it, and gives the complete label/card the selected surface/border/inset and focus ring. It does not add a circular checkbox substitute or decorative tick.
- An independent boolean such as Decade overview, Individual years, or Genre breakdown keeps a visible conventional checkbox or switch. Its containing row remains visually neutral rather than looking like a selected entity card.
- Teal/cyan is the ordinary selection language. Green remains reserved for meaningful success and does not become an alternative selected state. Selected state must remain structurally distinguishable in grayscale and forced-colours mode; hue alone is insufficient.
- A coloured leading rail is never a selection indicator. Notices, warnings, errors and explanatory messages also use no emphasized left edge. Follow the owner-approved People warning treatment: subtle semantic tint and an even thin border around the whole message. Warning surfaces reuse the shared amber treatment; information remains cyan and errors remain red. Keep compact spacing, accessible text, semantic roles and actions. Structural hierarchy-diagram lines and drag-placement indicators retain their existing purpose.

The shared `choice-card-input` treatment owns visually hidden full-card controls, contained positioning, and label-level focus. `tests/builder-choice-presentation-contract.test.mjs` is the test-owned family and presentation registry: every guided family must retain explicit evidence, obsolete marker tokens stay absent, full-card controls retain native semantics, independent booleans remain visible, forced-colours support remains present, and selected-state rails or green alternatives cannot be reintroduced accidentally.
...
Issue #113 routed every existing `New collection` and `New folder` entry point through one contextual dialog, recorded at the time with the historical heading **What would you like to create?**. Current production asks **What collection would you like to create?** or **What folder would you like to create?** according to scope. The ordered registry now exposes **Blank**, **Decades**, **People**, **Franchises**, **TMDB Lists**, **Studios**, **Networks**, **Genres**, then **Streaming**, with explicit per-scope support and fail-closed unknown-family handling. Blank immediately delegates to the exact existing draft helper and therefore preserves the defaults, naming, desktop selection, mobile level, and card-scrolling behavior above without adding a naming or review step.
...
People reuses that same modal lifecycle but keeps its People-specific search/provider/result cards and source controls. Search may receive focus only after the user explicitly chooses People (including the compatibility shortcut). Ordered selection has no hard maximum; each full result card remains clickable and contains a visually hidden, focusable native checkbox while the complete card carries selected surface/border/inset and focus, one compact count/action summary opens a collapsed bounded removable list, and at 50 it shows a nonblocking informational notice. Configure loads details only for selected people, exposes only Automatic and Same for all, and keeps every selected person visible in one ordered bounded compact list that consumes the available vertical space. All four marker-free source pills stay visible in the shared control and in every person row; an individual edit records an internal override without changing the visible strategy, while later shared changes affect unmodified people and preserve explicit overrides. The shared three-option People sort drives all generated sources; Most votes remains absent because it has no retained `PERSON`/`DIRECTOR` contract evidence. Each row may open one centred body-portalled poster-only title-preview modal on the shared nested layer above creation, bounded to 10 posters at every viewport with phone 5×2 layout, focus containment, Close/Escape/exact-trigger restoration and loading/empty/retry states. No dropdown or expanded person editor is mounted. The hierarchy flow uses **Step 1 · Select**, **Step 2 · Configure**, and **Step 3 · Review & Appearance**. Review shows ordinary hierarchy counts, then one shared **Title options** section followed by direct **Layout** controls for New Collection. Title options contain collection title visibility plus the three existing generated-folder outcomes with **Hide on Home screen only** selected by default; Layout contains Tabs/Rows, Tabs-specific Show All, and Pin. Rows hides Show All and retains `showAllTab: true`. New Folder exposes only the same generated-folder title outcomes, states **Collection settings stay unchanged.**, shows captured inherited presentation, and never patches the parent. Both scopes expose one shared Poster/Landscape control for generated folders and concise guidance that all remaining artwork uses canonical defaults and can be customised later through ordinary Folder editing; only the potentially large person detail remains collapsed. Neutral status rows distinguish ready, destination complete/partial, and elsewhere outcomes. Create applies one revalidated atomic family plan. Add Source → People continues using its Search → Configure path without Review or hierarchy duplicate policy.
...
The existing panel-header `New collection` and `New folder` controls remain. A real button with the same label appears below the final populated collection or folder card. These buttons open the same contextual launcher as their header and empty-state counterparts. Choosing Blank then uses the exact existing defaults, automatic naming, viewport-aware selection, and card scrolling. They are not list items, cards, reorder siblings, or drag targets. Empty panels retain their larger first-item `+` control with the matching `New collection` or `New folder` accessible name.

When a folder is selected, Sources exposes `Add source` in the header, empty state, and after a populated list. On mobile, the count is part of the `Sources · N` heading so Add source retains room; desktop retains the separate count badge. No disabled or ambiguous Add action appears without a selected folder. All three entry points open the same responsive mode picker for eight ordered modes: Movie franchise, TMDB lists, People, Studios, Networks, Streaming, Genres, and singular Decade. The picker shares the Creation launcher's narrow card/icon presentation seam, responsive grid, and focus-visible language while retaining immediate-action buttons with no Blank option or selection state. Provider context remains source-specific rather than making a blanket TMDB claim. Each child flow has visible Back navigation and preserves its own reviewed state; returning restores focus to the family card that launched it. The shared mobile Genre context/catalogue shell uses scoped browser history so browser Back returns from detail to context. Family-specific construction, duplicate, request, and review contracts remain in their focused Builder documents.
...
Issue [#192](https://github.com/davecollections/tmdb-id-lookup/issues/192) derives Workspace Source details through the Source-only `source-details.js` formatter. An existing visible title remains independent of configuration. Blank native names use the recognized family fallback: `TMDB List source`, `Movie franchise source`, `Studio source`, `Network source`, `Acting source`, `Directing source`, `Decade source`, `Year source`, `Genre source`, `Streaming source`, or `TMDB Discover source`. Addons retain catalog ID → addon ID → `Addon source`; opaque sources retain `Preserved source`. Shared and unusual-format title semantics are unchanged.

Workspace Sources are navigation cards. Visible details normally contain at most three segments, with no numeric TMDB entity/provider IDs or repeated category wording. Recognized periods and Genres combine with media (`1980s comedy movies · Popular`, `Comedy movies · Popular`); People use `Person · Acting movies · Popular` or `Person · Directed series · Popular`; Studios/Networks use kind, media and sort. Streaming uses `Streaming · Movies (AU) · Popular`. Franchises use `Movie franchise · TMDB order`; Lists use `List · Original order` and still omit media because persisted MOVIE does not describe mutable List contents. Generic Discover uses `Discover · Recent movies · Additional settings`. Keep existing family-specific sort tables and strict shape inspectors. Unknown sorting/media remains `Other sorting` / `Other media type`. Remaining meaningful filters and uninterpreted preserved fields produce one `Additional settings` indicator. Where necessary, combine sort with media/role to retain the three-segment target. No hidden accessibility-only version retains older verbose details.

Addon details omit the redundant Addon prefix. Exact `aio-metadata` maps to `AIO Metadata`; its explicit `trakt.` catalog namespace maps to `Trakt` (evidenced by the [source-authored collection example](https://gist.github.com/ImKaptain/81cf17a4efafdd5c507c9169e73da6ea), inspected 2026-09-05; no external code or layouts reused). Other addon identifiers remain exact. Omit a catalog already shown as the visible/fallback title, or summarized by the recognized origin. Preserve a necessary unknown catalog as `Catalog: …`. Combine useful Genre/media text, omit role-aware media repetition, and reuse the existing exact no-Genre aliases without mutating stored data. Only otherwise identical sibling addon cards with different catalogs expand the compact summary to retain exact distinguishing catalog IDs. This is view-only disambiguation, not duplicate identity. Opaque `trakt` and `community` display `Trakt` and `Community`; unfamiliar providers retain a contextual prefix. No integration support, addon-name enrichment, projection joins, catalogue/manifest requests, model fields, or new dependencies are introduced.
...
- semantic presentation fieldsets with native radios whose full-card controls carry visible focus, plus labelled switches and conventional independent checkboxes, including a compact named Folder visibility group and selected visual tile-shape cards;
- named modal dialogs with contained focus, safe Escape cancellation, inert background, and exact trigger focus restoration;
- one responsive Add Source mode chooser plus eight family-specific, focus-contained staged flows with visible Back navigation and family-specific validation and duplicate handling;
