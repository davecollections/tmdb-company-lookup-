...
### Queued after this refinement, no new issues created

1. **Next substantial investigation: shared Advanced options.** Cover every TMDB creation family except Lists and Franchises. Aim for all applicable Discover content filters with consistent creation/editing controls and exact Preview behavior: votes, ratings, dates, genres, languages, countries, keywords, runtime and availability where supported. Map TMDB capability, Nuvio serialized fields and actual client consumption before promising support. Preserve each category's constraints, including Studio, Network, period and exact Acting/Directing role. Investigate the inspected NuvioTV native People resolver's credits path, which does not apply Discover filters; do not silently convert People into a different source family. Reuse the resulting foundation for future standalone Advanced Discover. Warner Bros. Top rated correctly returning one-vote ratings motivates configurable options, not a hidden threshold.
2. **Export-help refinement.** Inside the existing expanded export help, offer method buttons **Nuvio.tv / TV app / TV via phone or computer** and switch one instruction panel within the existing dialog. TV manager instructions should cover scanning the QR code or entering the full address beneath it on a device connected to the same network, then **Collections > Import > Paste or File > Import > Save Changes**. Explain that URL requires an already hosted JSON file. Preserve shared beta and TMDB Enrichment guidance.
3. **Upstream Nuvio.tv report.** Report silently changed imported People Most voted sorting, with the absent website selector as related context. Use the supplied website, synced-TV, direct-TV, Desktop and manually created Top rated comparisons without asserting a confirmed internal rewriting stage. No upstream post is authorized in this task.
